      ...
      <main className="flex-1 overflow-y-auto p-8 space-y-6">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl border shadow-sm space-y-4">
            {/* ★ 핵심: Gemini API Key 입력란 ★ */}
            <div className="bg-amber-50 p-3.5 rounded-lg border border-amber-200">
              <label className="block text-xs font-bold text-amber-900 mb-1">
                🔑 Gemini API Key (개인 키 입력)
              </label>
              <input
                type="password"
                value={userApiKey}
                onChange={handleApiKeyChange}
                placeholder="AIzaSy... (본인의 Gemini API 키를 입력하세요)"
                className="w-full p-2.5 text-xs border border-amber-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-amber-500"
              />
              <p className="text-[10px] text-amber-700 mt-1">
                * 입력한 API 키는 브라우저에 안전하게 저장되며, 본인 전용으로 사용됩니다.
              </p>
            </div>
